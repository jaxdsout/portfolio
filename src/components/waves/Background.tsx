...
type ColorScheme = [string, string, string, string, string, string, string];

const defaultScheme: ColorScheme = ['#e6a157', '#e79b52', '#e8954d', '#e98f49', '#ea8945', '#eb8242', '#e7a55e'];

function getScheme(id: string | undefined): ColorScheme {
...
export default function Background(): JSX.Element {
  ...
  const id = location.pathname.match(/^\/proj\/(\d+)$/)?.[1];
  const [scheme, setScheme] = useState<ColorScheme>(getScheme(id));
  const prevScheme = useRef<ColorScheme>(scheme);
  const [p, setP] = useState(0);

  useEffect(() => {
    prevScheme.current = scheme;
    setScheme(getScheme(id));
    setP(0);
  ...
  const blended0 = interpolateRgb(prevScheme.current[0], scheme[0])(p);
  const blended1 = interpolateRgb(prevScheme.current[1], scheme[1])(p);
  const blended2 = interpolateRgb(prevScheme.current[2], scheme[2])(p);
  const blended3 = interpolateRgb(prevScheme.current[3], scheme[3])(p);
  const blended4 = interpolateRgb(prevScheme.current[4], scheme[4])(p);
